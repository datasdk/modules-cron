...
<div>
    <div class="content-header mb-3">
        <h1>Create New Cron Task</h1>
    </div>

    <form method="POST" action="{{ route('cron.tasks.store') }}">
        @csrf
                    ...
                    <td>
                        <select class="form-control" id="command" name="command" required>
                            ...
                            @foreach($commands as $command)
                                <option value="{{ $command['name'] }}" {{ old('command') == $command['name'] ? 'selected' : '' }}>
                                ...
                                <div class="col-md-4 mb-2">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="frequencies[]" 
                                               ...
                                               {{ in_array($frequency['interval'], old('frequencies', [])) ? 'checked' : '' }}>
                    ...
                    <td>
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" name="run_in_maintenance" id="run_in_maintenance" value="1"
                                   {{ old('run_in_maintenance') ? 'checked' : '' }}>
                ...
                <tr>
                    ...
                    <td>
                        <input type="email" class="form-control" id="notification_email_address" name="notification_email_address" 
                               value="{{ old('notification_email_address') }}">
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" class="form-control" id="notification_phone_number" name="notification_phone_number" 
                               value="{{ old('notification_phone_number') }}">
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" class="form-control" id="notification_slack_webhook" name="notification_slack_webhook" 
                               value="{{ old('notification_slack_webhook') }}">
                    </td>
                </tr>
            </tbody>
        </table>
                    ...
                    <td>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="auto_cleanup_type" id="cleanup_results" value="results"
                                   {{ old('auto_cleanup_type') == 'results' ? 'checked' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="auto_cleanup_type" id="cleanup_days" value="days"
                                   {{ old('auto_cleanup_type') == 'days' ? 'checked' : '' }}>
                        ...
                        <div class="row align-items-center">
                            <div class="col-auto">
                                <input type="number" class="form-control" style="width: 100px;" id="auto_cleanup_num" name="auto_cleanup_num" 
                                       value="{{ old('auto_cleanup_num') }}" min="0">
                            </div>
                            <div class="col">
...
document.addEventListener('DOMContentLoaded', function() {
    // Make sure is_active is checked by default for new tasks
    if (!document.querySelector('input[name="is_active"]:checked')) {
        document.querySelector('input[name="is_active"]').checked = true;
    }
    
    // Set default timezone to UTC if not already set
    if (!document.querySelector('#timezone').value) {
        document.querySelector('#timezone').value = 'UTC';
    }
});
</script>
@endsection